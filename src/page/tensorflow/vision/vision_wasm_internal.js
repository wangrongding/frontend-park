...
const ModuleFactory = (() => {
  ...
  return function (ModuleFactory = {}) {
    const Module = typeof ModuleFactory !== 'undefined' ? ModuleFactory : {}
      ...
      init() {
        ...
        if (canvas) {
          canvas.requestPointerLock =
            canvas.requestPointerLock || canvas.mozRequestPointerLock || canvas.webkitRequestPointerLock || canvas.msRequestPointerLock || (() => {})
          canvas.exitPointerLock =
            document.exitPointerLock || document.mozExitPointerLock || document.webkitExitPointerLock || document.msExitPointerLock || (() => {})
      ...
      calculateMouseEvent(event) {
        if (Browser.pointerLock) {
          ...
          if (event.type === 'touchstart' || event.type === 'touchend' || event.type === 'touchmove') {
            ...
            adjustedX *= cw / rect.width
            adjustedY *= ch / rect.height
            const coords = {
              x: adjustedX,
              y: adjustedY,
            }
    ...
    const dlopenMissingError = 'To use dlopen, you need enable dynamic linking, see https://github.com/emscripten-core/emscripten/wiki/Linking'
    ...
    function HandleAllocator() {
      ...
      this.allocate = function (handle) {
        const id = this.freelist.pop() || this.allocated.length
    ...
    const emval_handles = new HandleAllocator()
    ...
    function count_emval_handles() {
      let count = 0
      for (let i = emval_handles.reserved; i < emval_handles.allocated.length; ++i) {
    ...
    function __emval_as(handle, returnType, destructorsRef) {
      ...
      const destructors = []
      const rd = Emval.toHandle(destructors)
    ...
    function emval_get_global() {
      ...
      function testGlobal(obj) {
        ...
        const success = typeof $$$embind_global$$$ === 'object' && obj.$$$embind_global$$$ == obj
        if (!success) {
    ...
    function __gmtime_js(time, tmPtr) {
      const date = new Date(readI53FromI64(time) * 1e3)
    ...
    function __mmap_js(len, prot, flags, fd, off, allocated, addr) {
      try {
        const stream = SYSCALLS.getStreamFromFD(fd)
        const res = FS.mmap(stream, len, off, prot, flags)
        const ptr = res.ptr
    ...
    function __tzset_js(timezone, daylight, tzname) {
      ...
      const winterNamePtr = stringToNewUTF8(winterName)
      const summerNamePtr = stringToNewUTF8(summerName)
    ...
    function saveInUnwindCache(callstack) {
      callstack.forEach((frame) => {
        const pc = convertFrameToPC(frame)
        if (pc) {
    ...
    function _emscripten_stack_snapshot() {
      const callstack = jsStackTrace().split('\n')
    ...
    var GL = {
      ...
      generateTempBuffers(quads, context) {
        const largestIndex = GL.log2ceilLookup(GL.MAX_TEMP_BUFFER_SIZE)
        ...
        for (var i = 0; i <= largestIndex; ++i) {
          ...
          const ringbufferLength = GL.numTempVertexBuffersPerSize
          context.tempVertexBuffers1[i] = []
          context.tempVertexBuffers2[i] = []
          const ringbuffer1 = context.tempVertexBuffers1[i]
          const ringbuffer2 = context.tempVertexBuffers2[i]
        ...
        if (quads) {
          ...
          const numIndexes = GL.MAX_TEMP_BUFFER_SIZE >> 1
          const quadIndexes = new Uint16Array(numIndexes)
          var i = 0
          let v = 0
          while (1) {
            quadIndexes[i++] = v
      ...
      getTempVertexBuffer: function getTempVertexBuffer(sizeBytes) {
        const idx = GL.log2ceilLookup(sizeBytes)
        const ringbuffer = GL.currentContext.tempVertexBuffers1[idx]
        const nextFreeBufferIndex = GL.currentContext.tempVertexBufferCounters1[idx]
        ...
        const vbo = ringbuffer[nextFreeBufferIndex]
        if (vbo) {
          return vbo
        }
        const prevVBO = GLctx.getParameter(34964)
      ...
      getTempIndexBuffer: function getTempIndexBuffer(sizeBytes) {
        const idx = GL.log2ceilLookup(sizeBytes)
        const ibo = GL.currentContext.tempIndexBuffers[idx]
        if (ibo) {
          return ibo
        }
        const prevIBO = GLctx.getParameter(34965)
      ...
      newRenderingFrameStarted: function newRenderingFrameStarted() {
        ...
        let vb = GL.currentContext.tempVertexBuffers1
      ...
      getSource(shader, count, string, length) {
        ...
        for (let i = 0; i < count; ++i) {
          const len = length ? HEAP32[(length + i * 4) >> 2] : -1
      ...
      calcBufLength: function calcBufLength(size, type, stride, count) {
        ...
        const typeSize = GL.byteSizeByType[type - GL.byteSizeByTypeRoot]
      ...
      preDrawHandleClientVertexAttribBindings: function preDrawHandleClientVertexAttribBindings(count) {
        ...
        for (let i = 0; i < GL.currentContext.maxVertexAttribs; ++i) {
          const cb = GL.currentContext.clientBuffers[i]
          if (!cb.clientside || !cb.enabled) continue
          GL.resetBufferBinding = true
          const size = GL.calcBufLength(cb.size, cb.type, cb.stride, count)
          const buf = GL.getTempVertexBuffer(size)
    ...
    var JSEvents = {
      ...
      deferCall(targetFunction, precedence, argsList) {
        ...
        for (const i in JSEvents.deferredCalls) {
          const call = JSEvents.deferredCalls[i]
        ...
        JSEvents.deferredCalls.sort((x, y) => {
      ...
      registerOrRemoveHandler(eventHandler) {
        const jsEventHandler = function jsEventHandler(event) {
    ...
    const emscripten_webgl_power_preferences = ['default', 'low-power', 'high-performance']
    ...
    function _emscripten_webgl_do_create_context(target, attributes) {
      ...
      const powerPreference = HEAP32[a + (24 >> 2)]
      const contextAttributes = {
        alpha: !!HEAP32[a + (0 >> 2)],
        depth: !!HEAP32[a + (4 >> 2)],
    ...
    function _emscripten_webgl_get_context_attributes(c, a) {
      ...
      const power = t.powerPreference && emscripten_webgl_power_preferences.indexOf(t.powerPreference)
      HEAP32[(a + 24) >> 2] = power
    ...
    const _emscripten_webgl_get_current_context = _emscripten_webgl_do_get_current_context

    function _emscripten_webgl_init_context_attributes(attributes) {
      const a = attributes >> 2
      for (let i = 0; i < 56 >> 2; ++i) {
        HEAP32[a + i] = 0
      }
    ...
    var WebGPU = {
      ...
      makeTextureDataLayout(ptr) {
        const bytesPerRow = HEAPU32[(ptr + 16) >> 2]
        const rowsPerImage = HEAPU32[(ptr + 20) >> 2]
        return {
      ...
      makePipelineConstants(constantCount, constantsPtr) {
        ...
        for (let i = 0; i < constantCount; ++i) {
          const entryPtr = constantsPtr + 16 * i
          const key = UTF8ToString(HEAPU32[(entryPtr + 4) >> 2])
    ...
    function _glBindVertexArray(vao) {
      ...
      const ibo = GLctx.getParameter(34965)
    ...
    function _glClientWaitSync(sync, flags, timeout_low, timeout_high) {
      const timeout = convertI32PairToI53(timeout_low, timeout_high)
    ...
    function _glDeleteFramebuffers(n, framebuffers) {
      for (let i = 0; i < n; ++i) {
        const id = HEAP32[(framebuffers + i * 4) >> 2]
        const framebuffer = GL.framebuffers[id]
    ...
    function _glDisableVertexAttribArray(index) {
      const cb = GL.currentContext.clientBuffers[index]
      cb.enabled = false
    ...
    const tempFixedLengthArray = []

    function _glDrawBuffers(n, bufs) {
      const bufArray = tempFixedLengthArray[n]
      for (let i = 0; i < n; i++) {
    ...
    function _glFenceSync(condition, flags) {
      const sync = GLctx.fenceSync(condition, flags)
      if (sync) {
        const id = GL.getNewId(GL.syncs)
        sync.name = id
        GL.syncs[id] = sync
        return id
      }
    ...
    function _glGetError() {
      const error = GLctx.getError() || GL.lastError
      GL.lastError = 0
      return error
    }
      ...
      if (!ret) {
        switch (name_) {
          ...
          case 35724:
            ...
            if (ver_num !== null) {
              if (ver_num[1].length == 3) ver_num[1] += '0'
    ...
    function webglPrepareUniformLocationsBeforeFirstUse(program) {
      let uniformLocsById = program.uniformLocsById
      const uniformSizeAndIdsByName = program.uniformSizeAndIdsByName
      let i
      let j
      if (!uniformLocsById) {
    ...
    function emscriptenWebGLGetTexPixelData(type, format, width, height, pixels, internalFormat) {
      ...
      const shift = heapAccessShiftForWebGLHeap(heap)
      const byteSize = 1 << shift
      const sizePerPixel = colorChannelsInGlTextureFormat(format) * byteSize
      const bytes = computeUnpackAlignedImageSize(width, height, sizePerPixel, GL.unpackAlignment)
    ...
    function _wgpuBufferGetMappedRange(bufferId, offset, size) {
      ...
      const data = _malloc(mapped.byteLength)
    ...
    function _wgpuCommandEncoderBeginComputePass(encoderId, descriptor) {
      ...
      if (descriptor) {
        ...
        const timestampWriteCount = HEAPU32[(descriptor + 8) >> 2]
    ...
    function _wgpuCommandEncoderBeginRenderPass(encoderId, descriptor) {
      function makeColorAttachment(caPtr) {
        const viewPtr = HEAPU32[caPtr >> 2]
        if (viewPtr === 0) {
          return undefined
        }
        const loadOpInt = HEAPU32[(caPtr + 8) >> 2]
        const storeOpInt = HEAPU32[(caPtr + 12) >> 2]
        const clearValue = WebGPU.makeColor(caPtr + 16)
        return {
      ...
      function makeRenderPassDescriptor(descriptor) {
        const nextInChainPtr = HEAPU32[descriptor >> 2]
        let maxDrawCount
        if (nextInChainPtr !== 0) {
          const sType = HEAPU32[(nextInChainPtr + 4) >> 2]
        ...
        const labelPtr = HEAPU32[(descriptor + 4) >> 2]
        if (labelPtr) desc.label = UTF8ToString(labelPtr)
        const timestampWriteCount = HEAPU32[(descriptor + 24) >> 2]
        if (timestampWriteCount) {
    ...
    function _wgpuDeviceCreateBindGroup(deviceId, descriptor) {
      function makeEntry(entryPtr) {
        const bufferId = HEAPU32[(entryPtr + 8) >> 2]
        const samplerId = HEAPU32[(entryPtr + 32) >> 2]
        const textureViewId = HEAPU32[(entryPtr + 36) >> 2]
        const binding = HEAPU32[(entryPtr + 4) >> 2]
        if (bufferId) {
          const size_low = HEAP32[(entryPtr + 24) >> 2]
          const size_high = HEAP32[(entryPtr + 28) >> 2]
          const size = size_high === -1 && size_low === -1 ? undefined : (size_high >>> 0) * 4294967296 + (size_low >>> 0)
          return {
            binding,
            resource: {
    ...
    function _wgpuDeviceCreateBuffer(deviceId, descriptor) {
      const mappedAtCreation = HEAP8[(descriptor + 24) >> 0] !== 0
      const desc = {
        label: undefined,
    ...
    function _wgpuDeviceCreateRenderPipeline(deviceId, descriptor) {
      ...
      function makeColorState(csPtr) {
        const formatInt = HEAPU32[(csPtr + 4) >> 2]
        return formatInt === 0
          ? undefined
          : {
      ...
      function makeVertexBuffer(vbPtr) {
        ...
        const stepModeInt = HEAPU32[(vbPtr + 8) >> 2]
    ...
    function _wgpuDeviceCreateShaderModule(deviceId, descriptor) {
      ...
      switch (sType) {
        case 5: {
          const count = HEAPU32[(nextInChainPtr + 8) >> 2]
          const start = HEAPU32[(nextInChainPtr + 12) >> 2]
          desc.code = HEAPU32.subarray(start >> 2, (start >> 2) + count)
          break
        }

        case 6: {
          const sourcePtr = HEAPU32[(nextInChainPtr + 8) >> 2]
          if (sourcePtr) {
    ...
    function _wgpuDeviceCreateTexture(deviceId, descriptor) {
      ...
      if (viewFormatCount) {
        ...
        desc.viewFormats = Array.from(HEAP32.subarray(viewFormatsPtr >> 2, (viewFormatsPtr >> 2) + viewFormatCount), (format) => {
    ...
    function _wgpuQueueSubmit(queueId, commandCount, commands) {
      ...
      const cmds = Array.from(HEAP32.subarray(commands >> 2, (commands >> 2) + commandCount), (id) => {
    ...
    function _wgpuQueueWriteBuffer(queueId, bufferId, bufferOffset_low, bufferOffset_high, data, size) {
      ...
      const bufferOffset = (bufferOffset_high >>> 0) * 4294967296 + (bufferOffset_low >>> 0)
    ...
    const miniTempWebGLIntBuffersStorage = new Int32Array(288)
    ...
    const ___start_em_js = (Module.___start_em_js = 1204068)

    const ___stop_em_js = (Module.___stop_em_js = 1210616)
